...
def init_fake_datas():
    ...
    with db:
        ...
        with open(os.path.join(C, "mock_cp.txt"), "r", encoding="utf8") as f_in:
            data = f_in.readlines()
        cp = dict()
        for line in data:
        ...
        hotels = []
        with open(os.path.join(C, "mock_hotel.csv"), "r", encoding="utf8") as f_in:
        ...
        nom = set()
        prenom = set()
        with open(os.path.join(C, "mock_name.csv"), "r", encoding="utf8") as f_in:
            reader = csv.reader(f_in)
            reader.__next__()
            for row in reader:
                nom.add(row[1])
